ST.ARCAR <- function(formula, data=NULL, W, burnin=0, n.sample=1000, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, verbose=TRUE)
{
...
N <- N.all / K
  ...
  phi <- matrix(rnorm(N.all), nrow=K, ncol=N, byrow=FALSE)
  tau2 <- runif(1)
  rho <- runif(1)
  gamma <- runif(1)
...
#### Specify quantities that do not change
  ...
  ## Start timer
    ...
    ## Sample from phi
    ####################
    phi.offset <- exp(offset.mat + regression.mat)
    ...
    accept[6] <- accept[6] + n.beta.block    
    regression.mat <- matrix(X.standardised %*% beta, nrow=K, ncol=N, byrow=FALSE)   
    ...
    #########################
    fitted <- as.numeric(exp(offset.mat + regression.mat + phi))
...
rownames(summary.hyper) <- c("tau2", "rho", "gamma")     
...
## Compile and return the results
  ...
  samples <- list(beta=mcmc(samples.beta.orig), phi=mcmc(samples.phi),  rho=mcmc(samples.rho), tau2=mcmc(samples.tau2), gamma=mcmc(samples.gamma), fitted=mcmc(samples.fitted))
