binomial.CARanova <- function(formula, data=NULL, trials, W, interaction=TRUE, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, verbose=TRUE)
{
...
samples.deviance <- array(NA, c(n.keep, 1))
    ...
    failures.vec <- as.numeric(failures)
    ...
    for(j in 1:n.sample)
    {
        ...
        temp1 <- binomialcarupdate(W.triplet, W.begfin, W.triplet.sum, K, phi, tau2.phi,Y.mat, failures.mat, proposal.sd.phi, rho, phi.offset, N, rep(1,N))
        phi <- temp1[[1]]
        phi <- phi - mean(phi)
        phi.mat <- matrix(rep(phi, N), byrow=F, nrow=K)    
        ...
        temp2 <- binomialcarupdate(D.triplet, D.begfin, D.triplet.sum, N, delta, tau2.delta, Y.mat.trans, failures.mat.trans, proposal.sd.delta, lambda, delta.offset, K, rep(1,K))
        delta <- temp2[[1]]
        ...
        if(interaction)
        {
            ...
            temp5 <- binomialindepupdate(N.all, gamma, tau2.gamma, Y, failures, proposal.sd.gamma, gamma.offset.vec)
            gamma <- temp5[[1]]
...
accept.lambda <- 100 * accept.all[11] / accept.all[12]
    ...
    names(accept.final) <- c("beta", "phi", "delta", "gamma", "rho", "lambda")
    }else
    {
...
regression.mat <- matrix(X.standardised %*% median.beta, nrow=K, ncol=N, byrow=FALSE)   
    ...
    lp.median <- as.numeric(offset.mat + regression.mat + median.phi.mat + median.delta.mat + median.gamma.mat)
    }else
    {
    lp.median <- as.numeric(offset.mat + regression.mat + median.phi.mat + median.delta.mat)   
    }
...
if(interaction)
{
    ...
    rownames(summary.hyper) <- c("tau2.phi", "tau2.delta", "tau2.gamma", "rho", "lambda")     
...
}else
{
    ...
    rownames(summary.hyper) <- c("tau2.phi", "tau2.delta",  "rho.phi", "rho.delta")     
    summary.hyper[1, 4:7] <- c(n.keep, 100, effectiveSize(mcmc(samples.tau2[ ,1])), geweke.diag(mcmc(samples.tau2[ ,1]))$z)     
    summary.hyper[2, 4:7] <- c(n.keep, 100, effectiveSize(mcmc(samples.tau2[ ,2])), geweke.diag(mcmc(samples.tau2[ ,2]))$z)   
    summary.hyper[3, 4:7] <- c(n.keep, accept.rho, effectiveSize(mcmc(samples.rho)), geweke.diag(mcmc(samples.rho))$z)   
    summary.hyper[4, 4:7] <- c(n.keep, accept.lambda, effectiveSize(mcmc(samples.lambda)), geweke.diag(mcmc(samples.lambda))$z)     
}
...
samples.rhoext <- cbind(samples.rho, samples.lambda)
    ...
    if(interaction)
    {
        samples <- list(beta=mcmc(samples.beta.orig), phi=mcmc(samples.phi),  delta=mcmc(samples.delta), gamma=mcmc(samples.gamma), tau2=mcmc(samples.tau2), rho=mcmc(samples.rhoext), fitted=mcmc(samples.fitted))        
    ...
    {
        samples <- list(beta=mcmc(samples.beta.orig), phi=mcmc(samples.phi),  delta=mcmc(samples.delta), tau2=mcmc(samples.tau2), rho=mcmc(samples.rhoext), fitted=mcmc(samples.fitted))        
