binomial.CARanova <- function(formula, data=NULL, trials, W, interaction=TRUE, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, fix.rho.S=FALSE, rho.S=NULL, fix.rho.T=FALSE, rho.T=NULL,verbose=TRUE)
{
...
if(n.miss>0) samples.Y <- array(NA, c(n.keep, n.miss))
    ...
    failures.mat.trans.miss <- t(failures.mat.miss)
    ...
    for(j in 1:n.sample)
    {
        ...
        temp2 <- binomialcarupdate(D.triplet, D.begfin, D.triplet.sum, N, delta, tau2.delta, Y.mat.trans.miss, failures.mat.trans.miss, proposal.sd.delta, lambda, delta.offset, K, rep(1,K), which.miss.mat.trans)
        delta <- temp2[[1]]
        ...
        if(interaction)
        {
            ...
            temp5 <- binomialindepupdate(N.all, gamma, tau2.gamma, Y.miss, failures.miss, proposal.sd.gamma, gamma.offset.vec, which.miss)
            gamma <- temp5[[1]]
