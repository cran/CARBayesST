ST.clustconv <- function(formula, data=NULL, coordinates=NULL, G, burnin=0, n.sample=1000, thin=1,  blocksize.beta=5, prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, prior.sigma2=NULL, prior.alpha=NULL, prior.rho=NULL, verbose=TRUE)
{
...
if(is.null(prior.rho)) prior.rho <- 10       
if(length(prior.rho)!=1) stop("the prior value for rho is the wrong length.", call.=FALSE)    
if(!is.numeric(prior.rho)) stop("the prior value for rho is not numeric.", call.=FALSE)    
if(sum(is.na(prior.rho))!=0) stop("the prior value for rho has missing values.", call.=FALSE)    
if(prior.rho<=0) stop("the prior value for rho is not positive.", call.=FALSE)    
...
alpha <- runif(1, min=0, max=prior.alpha)
rho <- runif(n=1, min=1, max=prior.rho)
rho <- 0.01
mu <- array(NA, c(K,N))
     for(t in 1:N)
     {
...
tau2 <- runif(1)
X <- matrix(rnorm(n=(N*K), mean=0, sd=sqrt(tau2)), nrow=K, ncol=N)
...
#### Set up the Kernel weights
Dtemp <- as.matrix(dist(coordinates))     
D <- Dtemp / max(as.numeric(Dtemp))
W <- exp(-(D^2) / (2*rho))  
diag(W) <- rep(0,K)     
Wspace <- W  / apply(W,1,sum)     

#### Set up the weight matrix and convolution component
conv <- array(NA, c(K,N))
convprop <- array(NA, c(K,N))
     for(t in 1:N)
     {
     conv[ ,t] <- Wspace %*% X[ ,t]
     group.temp <- rep(0,G)
     group.mean <- tapply(conv[ ,t], Z[ ,t], mean)
     group.temp[as.numeric(names(group.mean))] <- group.mean     
     conv[ ,t] <- conv[ ,t]  - group.temp[Z[ ,t]]
     }
...
samples.conv <- array(NA, c(n.keep, (K*N)))
...
## Start timer
     ...
     #######################     
          ...
          lp.current <- lambdatemp[Z[ ,t]] + as.numeric(offset.mat[ ,t]) + as.numeric(conv[ ,t]) + as.numeric(regression.mat[ ,t])   
          lp.proposal <- proposal[Z[ ,t]] + as.numeric(offset.mat[ ,t]) + as.numeric(conv[ ,t]) + as.numeric(regression.mat[ ,t])
     ...
     #### Sample from X
     ##################
     offset.X <- exp(offset.mat + mu + regression.mat)
     temp1 <- Xupdate(Wspace=Wspace, Y=Y.mat, offset=offset.X, K=K, N=N, X=X, tau2=tau2, proposalsdX=proposal.sd.X)
     X <- temp1[[1]] 
     accept[6] <- accept[6] + N * K
     ...
     #### Sample from tau2
     #####################
     tau2.posterior.scale <- 0.5 * sum(X^2) + prior.tau2[2] 
     ...
     #### Sample from Z
     ##################       
     mu.offset <- exp(offset.mat + regression.mat + conv)
     ...
     ####################          
     #### Propose a new value
     proposal.rho <-  rtrunc(n=1, spec="norm", a=0.1, b=prior.rho, mean=rho, sd=proposal.sd.rho)          
     
     #### Compute the new convolution component
     W <- exp(-(D^2) / (2*proposal.rho))  
     diag(W) <- rep(0,K)     
     Wspaceprop <- W  / apply(W,1,sum)     
          for(t in 1:N)
          {
          convprop[ ,t] <- Wspaceprop %*% X[ ,t]
          group.temp <- rep(0,G)
          group.mean <- tapply(convprop[ ,t], Z[ ,t], mean)
          group.temp[as.numeric(names(group.mean))] <- group.mean     
          convprop[ ,t] <- convprop[ ,t]  - group.temp[Z[ ,t]]
          }
          
     #### Compute the acceptance probability     
     lp.current <- as.numeric(mu + offset.mat + regression.mat + conv)   
     lp.proposal <- as.numeric(mu + offset.mat + regression.mat + convprop)
     prob1 <- sum(as.numeric(Y.mat) * lp.current - exp(lp.current))
     prob2 <- sum(as.numeric(Y.mat) * lp.proposal - exp(lp.proposal))
     prob6 <- exp(prob2 - prob1)
               if(prob6 > runif(1))
               {
               accept[7] <- accept[7] + 1
               rho <- proposal.rho
    ...
    proposal.beta <- beta
    offset.temp <- as.numeric(conv) + offset + as.numeric(mu)
    ...
    regression.mat <- matrix(X.standardised %*% beta, nrow=K, ncol=N, byrow=FALSE)   
     ...
     #########################
     fitted <- as.numeric(exp(mu + offset.mat + regression.mat + conv))
     ...
     ###################
          ...
          samples.conv[ele, ] <- as.numeric(conv)
     ...
     k <- j/100
          ...
          accept.X <- 100 * accept[5] / accept[6]
          ...
          #### alpha tuning parameter               
               if(accept.alpha > 70)
               {
               proposal.sd.alpha <- min(2 * proposal.sd.alpha, prior.alpha / 6)
          ...
          #### X tuning parameter               
               if(accept.X > 70)
               {
               proposal.sd.X <- 2 * proposal.sd.X
               }else if(accept.X < 40)              
               {
               proposal.sd.X <- 0.5 * proposal.sd.X
               }else
               {
               }
          ...
          #### rho tuning parameter               
               if(accept.rho > 70)
               {
               proposal.sd.rho <- min(2 * proposal.sd.rho, prior.rho / 6)
...
accept.X <- 100 * accept.all[5] / accept.all[6]
accept.rho <- 100 * accept.all[7] / accept.all[7]
accept.beta <- 100 * accept.all[9] / accept.all[10]
accept.final <- c(accept.beta, accept.lambda, accept.alpha, accept.X, accept.rho)
names(accept.final) <- c("beta", "lambda", "alpha", "X", "rho")
...
median.conv <- matrix(apply(samples.conv, 2, median), nrow=K, ncol=N)
median.beta <- apply(samples.beta,2,median)
regression.mat <- matrix(X.standardised %*% median.beta, nrow=K, ncol=N, byrow=FALSE)   
fitted.median <- as.numeric(exp(median.mu + offset.mat + median.conv + regression.mat))
...
names(modelfit) <- c("DIC", "p.d", "DIC3", "WAIC", "p.waic", "LMPL")
     ...
     {
     samples <- list(lambda=mcmc(samples.lambda),  alpha=mcmc(samples.alpha), Z=mcmc(samples.Z), conv=mcmc(samples.conv), sigma2=mcmc(samples.sigma2), tau2=mcmc(samples.tau2), rho=mcmc(samples.rho), fitted=mcmc(samples.fitted))
     }else
     {
     samples <- list(beta=mcmc(samples.beta.orig[ ,-which(X.indicator==2)]), lambda=mcmc(samples.lambda),  alpha=mcmc(samples.alpha), Z=mcmc(samples.Z), conv=mcmc(samples.conv), sigma2=mcmc(samples.sigma2), tau2=mcmc(samples.tau2), rho=mcmc(samples.rho), fitted=mcmc(samples.fitted))
     }     
model.string <- c("Likelihood model - Poisson (log link function)", "\nLatent structure model - Clustering model with convolution random effects\n")
