gaussian.CARlinear <- function(formula, data=NULL, W, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.mean.alpha=NULL, prior.var.alpha=NULL, prior.nu2=NULL, prior.tau2=NULL, verbose=TRUE)
{
...
beta <- lm(Y~X.standardised-1 + time.all, offset=offset)$coefficients[1:p]
alpha <- lm(Y~X.standardised-1 + time.all, offset=offset)$coefficients[(p+1)]
...
nu2.phi <- runif(1, min=var(res.temp)/2, max=var(res.temp)*2)
rho <- runif(1)  
lambda <- runif(1)  
...
## Start timer
    ...
    ## Sample from nu2
    ##################
    nu2.offset <- as.numeric(Y.mat - offset.mat - regression.mat - phi.mat - delta.time.mat - alpha * time.mat)
    ...
    beta.offset <- as.numeric(Y.mat - offset.mat - phi.mat - delta.time.mat - alpha * time.mat)
    ...
    ## Sample from alpha
    ####################
    fc.var <- 1 / (1 / prior.var.alpha + K * sum(time^2) / nu2)
    alpha.offset <- Y.mat - offset.mat - regression.mat - phi.mat - delta.time.mat
    alpha.offset2 <- sum(alpha.offset / (nu2 / time.mat)) +  prior.mean.alpha / prior.var.alpha
    fc.mean <- fc.var * alpha.offset2
    ...
    ## Sample from phi
    ####################
    phi.offset <- Y.mat - offset.mat - regression.mat - delta.time.mat - alpha * time.mat
    ...
    ## Sample from delta
    ####################
    delta.offset <- Y.mat - offset.mat - regression.mat - phi.mat - alpha * time.mat
    delta.offset2 <- delta.offset * time.mat    
    delta.offset3 <- apply(delta.offset2,1, sum)
    delta.offset4 <- sum(time^2)
    temp2 <- gaussiancarupdate(W.triplet, W.begfin, W.triplet.sum, K, delta, tau2.delta, nu2, delta.offset3, lambda, delta.offset4)
    delta <- temp2
...
LP <- offset.mat + regression.mat + phi.mat + delta.time.mat + median.alpha * time.mat
fitted.median <- as.numeric(LP)
...
rownames(summary.hyper) <- c("alpha", "tau2.phi", "tau2.delta",  "nu2", "rho.phi", "rho.delta")     
...
summary.hyper[5, 4:7] <- c(n.keep, accept.rho, effectiveSize(mcmc(samples.rho)), geweke.diag(mcmc(samples.rho))$z)   
summary.hyper[6, 4:7] <- c(n.keep, accept.lambda, effectiveSize(mcmc(samples.lambda)), geweke.diag(mcmc(samples.lambda))$z)         
...
samples <- list(beta=mcmc(samples.beta.orig), alpha=mcmc(samples.alpha), phi=mcmc(samples.phi),  delta=mcmc(samples.delta), tau2=mcmc(samples.tau2), nu2=mcmc(samples.nu2), rho=mcmc(samples.rhoext), fitted=mcmc(samples.fitted))        
