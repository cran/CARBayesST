binomial.CARadaptive <- function(formula, data = NULL, trials, W, burnin, n.sample, thin = 1, prior.mean.beta = NULL, prior.var.beta = NULL, prior.tau2 = NULL, prior.zeta2 = NULL, verbose = TRUE, rhofix = NULL, epsilon = NULL)
    { 
  ...
  prior.max.tau <- 1000
  increment     <- 0
  glm_mod       <- glm(y/trials ~-1+X.standardised, family = "binomial", weights = trials, offset = offset)
  ...
  for(j in 1:n.sample){ 
    ...
    for(r in 1:n.beta.block){
      ...
      prob <- binomialbetaupdate(X=X.standardised, nsites=k, p=p, beta=beta_par, proposal=proposal.beta, 
                                offset=offset.temp, y=y, failures=failures, prior_meanbeta=prior.mean.beta, prior_varbeta=prior.var.beta)
      if(prob > runif(1)){
    ...
    prob         <- (1/(1 + exp( - as.numeric(X.standardised %*% beta_par) - phi)))
    fitted       <- prob*trials
  ...
  rownames(summary.hyper) <- c("tau2", "rho", "gamma", "zeta2")     
  ...
  samples         <- list(beta = mcmc(samples.beta.orig), phi = mcmc(samples.phi), rho = mcmc(samples.rho), 
                          tau2 = mcmc(samples.tau2), gamma=mcmc(samples.alpha), zeta2 = mcmc(samples.vtau2), 
                          samples.w = samples.w, fitted = mcmc(samples.fit))  
