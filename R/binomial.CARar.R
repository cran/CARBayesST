binomial.CARar <- function(formula, data=NULL, trials, W, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, verbose=TRUE)
{
...
phi.mat <- matrix(phi, nrow=K, ncol=N, byrow=FALSE)   
    ...
    for(j in 1:n.sample)
    {
        ...
        temp1 <- binomialarcarupdate(W.triplet, W.begfin, W.triplet.sum,  K, N, phi.mat, tau2, gamma, rho, Y.mat, failures.mat, proposal.sd.phi, phi.offset, den.offset)      
        phi.temp <- temp1[[1]]
