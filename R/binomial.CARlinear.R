binomial.CARlinear <- function(formula, data=NULL, trials, W, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.mean.alpha=NULL, prior.var.alpha=NULL, prior.tau2=NULL, verbose=TRUE)
{
...
beta <- glm(dat~X.standardised-1 + time.all, offset=offset, family=binomial)$coefficients[1:p]
alpha <- glm(dat~X.standardised-1 + time.all, offset=offset, family=binomial)$coefficients[(p+1)]
theta.hat <- Y / trials
...
## Start timer
    ...
    for(j in 1:n.sample)
    {
        ...
        temp2 <- binomialcarupdate(W.triplet, W.begfin, W.triplet.sum, K, delta, tau2.delta,Y.mat, failures.mat, proposal.sd.delta, lambda, delta.offset, N, time)
        delta <- temp2[[1]]
