...
if(!fix.rho.T)
{
  summary.hyper[4, 1:3] <- c(mean(samples.alpha[ ,1]), quantile(samples.alpha[ ,1], c(0.025, 0.975)))
  summary.hyper[4, 4:7] <- c(n.keep, 100, effectiveSize(mcmc(samples.alpha[ ,1])), geweke.diag(mcmc(samples.alpha[ ,1]))$z)
  summary.hyper[5, 1:3] <- c(mean(samples.alpha[ ,2]), quantile(samples.alpha[ ,2], c(0.025, 0.975)))
