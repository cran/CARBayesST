ST.knorrheld.main <- function(formula, data=NULL, W, burnin=0, n.sample=1000, thin=1,  blocksize.beta=5, prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, verbose=TRUE)
{
...
#### Set up matrices to store samples
  ...
  samples.tau2 <- array(NA, c(n.keep, 4))
  colnames(samples.tau2) <- c("tau2.phi", "tau2.theta", "tau2.alpha", "tau2.beta")
  ...
  #### Create the duplet form for the spatial W matrix
...
timelist[[N]] <- N-1     
  ...
  ## Start timer
    ...
    ## Sample from phi
    ####################
    phi.offset <- exp(offset.mat + regression.mat + theta.mat + alpha.mat + delta.mat)
    ...
    ## Sample from theta
    ####################
    theta.offset <- exp(offset.mat  + regression.mat + phi.mat + alpha.mat + delta.mat)
    ...
    ## Sample from alpha
    ####################
    alpha.offset <- exp(offset.mat + regression.mat + phi.mat + theta.mat + delta.mat)
    ...
    ## Sample from delta
    ####################
    delta.offset <- exp(offset.mat + regression.mat + phi.mat + theta.mat + alpha.mat)
    ...
    proposal.beta <- beta
    offset.temp <- offset + as.numeric(phi.mat) + as.numeric(theta.mat) + as.numeric(alpha.mat) + as.numeric(delta.mat)       
       
       for(r in 1:n.beta.block)
       {
       proposal.beta[beta.beg[r]:beta.fin[r]] <- proposal[beta.beg[r]:beta.fin[r]]
       prob <- poissonbetaupdate(X=X.standardised, nsites=N.all, p=p, beta=beta, proposal=proposal.beta, offset=offset.temp, y=Y, prior_meanbeta=prior.mean.beta, prior_varbeta=prior.var.beta)
    ...
    #########################
    fitted <- as.numeric(exp(offset.mat + regression.mat + phi.mat + theta.mat + alpha.mat + delta.mat))
    ...
    ###################
      ...
      samples.tau2[ele, ] <- c(tau2.phi, tau2.theta, tau2.alpha, tau2.delta)
...
# end timer
  ...
  accept.final <- c(accept.beta, accept.phi, accept.theta, accept.alpha, accept.delta)
  names(accept.final) <- c("beta", "phi", "theta", "alpha", "delta")
  ...
  fitted.median <- as.numeric(exp(offset.mat + regression.mat + median.space.mat + median.time.mat))
...
rownames(summary.hyper) <- c("tau2.phi", "tau2.theta", "tau2.alpha", "tau2.delta")     
...
## Compile and return the results
  ...
  samples <- list(beta=mcmc(samples.beta.orig), space=mcmc(samples.space),  time=mcmc(samples.time), tau2=mcmc(samples.tau2), fitted=mcmc(samples.fitted))
model.string <- c("Likelihood model - Poisson (log link function)", "\nLatent structure model - Convolution of spatial and temporal main effects\n")
