poisson.CARar <- function(formula, data=NULL, W, burnin, n.sample, thin=1,  prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, verbose=TRUE)
{
...
tau2 <- runif(1, min=var(res.temp)/2, max=var(res.temp)*2)
rho <- runif(1)
gamma <- runif(1)
...
phi.mat <- matrix(phi, nrow=K, ncol=N, byrow=FALSE)   
  ...
  ## Start timer
    ...
    temp1 <- poissonarcarupdate(W.triplet, W.begfin, W.triplet.sum,  K, N, phi.mat, tau2, gamma, rho, Y.mat, proposal.sd.phi, phi.offset, den.offset)      
    phi.temp <- temp1[[1]]
    ...
    #########################
    fitted <- as.numeric(exp(offset.mat + regression.mat + phi.mat))
...
summary.hyper[2, 4:7] <- c(n.keep, accept.rho, effectiveSize(mcmc(samples.rho)), geweke.diag(mcmc(samples.rho))$z)   
